...
print("Mean footprint: {} gm^2 a / kg beef".format(data["Footprint"].mean())) # ~620 gm^2 a / kg beef
print("20% footprint: {} m^2 a / kg beef".format(data["Footprint"].quantile(q=0.2))) # ~246 gm^2 a / kg beef
...
scatter.text(median, 3650, "50% (Median)", ha="center", **style)

# Aufteilung in Gesamtdatensatz und Datensatz bis zum Median
reduced = data[data['GHG Emissions Beef'] < median]
print("Pearson Correlation: {}".format(data['GHG Emissions Beef'].corr(data['Footprint']))) # 0.9 (Hohe Korrelation)
print("Pearson Correlation for GHG emissions below median: {}".format(reduced['GHG Emissions Beef'].corr(reduced['Footprint']))) # 0.39 (Geringe Korrelation)
